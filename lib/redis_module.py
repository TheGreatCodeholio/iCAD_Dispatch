...
Author: Ian Carey
Date: 2020-15-02
Description: A Redis module

Usage: In main script import and start RedisCache
`rd = RedisCache()`
...
module_logger = logging.getLogger('icad_dispatch.redis')
...
class RedisCache:
    ...
    Attributes:
        connection_pool (redis.ConnectionPool): Pool of Redis connections.
        client (redis.Redis): Redis client for operations.
    """

    def __init__(self):
        """
        Initialize the RedisCache with environment variables.
        ...
        required_redis_keys = ["REDIS_HOST", "REDIS_PORT", "REDIS_PASSWORD", "REDIS_CACHE_DB"]
        ...
        try:
            self.redis_db = int(os.getenv("REDIS_CACHE_DB"))
        except ValueError:
            raise ValueError("REDIS_CACHE_DB must be an integer.")
    ...
    def lrange(self, list_name, start=0, end=-1):
        ...
        try:
            ...
            deserialized_values = [self.deserialize_from_redis(value.decode("utf-8")) for value in values]
    ...
    def deserialize_from_redis(self, data):
        """
        Convert a string representation from Redis storage back to a Python data item.

        Args:
            data (str): The string representation from Redis storage.

        Returns:
            any: The original Python data item.
        """
        ...
        try:
            ...
            return self._convert_value(json_data)
    ...
    def serialize_for_redis(self, data):
        ...
        return json.dumps(self._convert_value(data))
